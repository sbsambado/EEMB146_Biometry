...
Week 7 Agenda Topics
1. Turn in HW 6 on Wednesday (5/13) by 11:59
2. HW 7 due next Wednesday (5/20) by 11:59
3. Please look at comments TAs give you on HW assignments!
4. How to email asking for help
  - take a screenshot of your Rmarkdown code AND error message 


```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE)
knitr::opts_knit$set(root.dir = "~/Desktop/Classes/EEMB 146 S20/Week 7/Lab /Lab 7 Data files-20200510") #to set your working directory for the whole document so you don't have to set it in each code chunk
...
(1) whether two variables are linearly associated with each other  
(2) how they are associated (positively, negatively, or neither). 

For example, I may be interested in whether large bodied animals have larger brains: 
> in other words, are body mass (*X*) and brain size (*Y*) correlated? 

To answer this question, I need to use something called *Pearson's correlation coefficient*, which is written as *r*. The *r* coefficient has the following properties:  
  
1. It has a value between -1 and 1 
...
We can test the hypothesis that there is a signficiant correlation between two variables *X* and *Y* by asking whether $\rho$ (the true value of *r*) is equal to zero or not: $$H_0 : \rho = 0 \\ H_A: \rho \neq 0$$  
...
colnames(plot.kelp)<-c("max_wid", "min_wid", "abund_amphipod") #rename since theyre logged now
...
colnames(log.plot.kelp)<-c("max_wid_log", "min_wid_log", "abund_amphipod_log") #rename since theyre logged now
pairs.panels(log.plot.kelp, density = TRUE, cor=FALSE, lm=TRUE)  # makes a scatterplot matrix of your dataset. show density plots, do not show correlation values, use a linear model


```  
You can see that the normality of the maximum and minimul kelp width is much better (we could test it with a Shapiro-Wilk). 

Moreover, there is no clear non-linear relationship that we can see between max_wid_log and min_wid_log. 

Therefore, let's proceed with a correlation test between these two variables:  
```{r}
# two variables = log of max width kelp and log of min width kelp
...
```{r}
# two variables = max width kelp & amphipod abundance
# amphipod abundance not normally distributed, assumptions not met, run non-parametric test
...
```
First, look at $\rho$. This is the Spearman's correlatioj coefficient and, like *r*, it varies from -1 to 1. The positive value tells us that there is a positive association between our variables. Next, look at the p-value for our hypothesis test: it is so small! Therefore, *we reject the H_0 that there is no significant correlation between the ranks of amphipod abundance and the ranks maximum kelp width*. 

In other words, *there tend to be more amphipods under larger kelp wracks*.  
...
* Predict *Y* using *X*  
 (this should be a mantra in your head when you think about the variables in your final project datasets)  
 
For example, if you were interested in determining whether a baby's age (*X*) was a signficiant predictor of a baby's ear length (*Y*), you would collect a bunch of (*X*,*Y*) pairs and build a regression model. 

From this model you could predict the expected ear length of a baby who was, say, 18 months old. What would this model look like?? What are the assumptions of the model??!! Proceed to the next section to find out!!  
...
2. Our $Y_i$'s are independent (not correlated to eachother).  
  + Meet this assumption via good experimental design.  
...
While we can test many hypotheses with a linear regression model, the most common hypotheses that we will test is whether *X* is a signficant predictor of *Y*. In other words, is the slope of our regression line different from 0? This is phrased as $$H_0: \beta_1 = 0 \\ H_A: \beta \neq 0$$.  
...
```{r}
deet <- read.csv("~/Desktop/Classes/EEMB 146 S20/Week 7/Lab /Lab 7 Data files-20200510/deet.csv")
...
Just like with ANOVA, the second plot (Normal Q-Q) tests our assumption of normality. We can see that the QQplot of our residuals does not deviate much from the straight line so we can conclude that our *assumption of normality is met*. 

If we then look at the first plot (Residuals vs Fitted), we see that there is no distinct pattern in the residuals about the 0 line. Therefore, *our equal variance assumption is met*. We could do quantitative diagnostics if we wanted to, but these qualitative diagnostics looks pretty good. If our assumptions were not met we could try some transformations.  
...
2. Look at the column *Pr(>|t|) for your p-values.  
  + Remember that our $H_0$ for linear regression is that our $\beta_1 = 0$. In other words, is *dose* a signficiant predictor of *bites*?  
...
Use R Markdown to answer these questions and upload your answers as HTML or PDF in the Lab 8 turn-in link on Gauchospace. The assignment is, as always, due next week before your lab.  
  
#### Exercise 1: correlating dandelions  
...
```{r}
plant <- read.csv("~/Desktop/Classes/EEMB 146 S20/Week 5/Lab 5/Lab 5 Data-20200427/plant_data.csv")
View(plant)
...
```{r}
plot.plant$num_leaves_log <- log(plot.plant$num_leaves_in_rosette+1)
colnames(plot.plant) <- c('num_leaves', 'dand_diam', 'num_leaves_log')
pairs.panels(plot.plant)
```
  
4. Run a Pearson's correlation test on the transformed data.  
```{r}
# i just added log data column to plot.plant, make sure to use num_leaves_LOG
cor.test(plot.plant$num_leaves_log, plot.plant$dand_diam, 
         method = 'pearson',
         alternative = 'two.sided')

# cor is  positive correlation
# p < 0.05, reject null, p is significant correlation between leaves ~ diam
```
  
5. Run a Spearman's rank correlation test on the untransformed data.  
```{r}
# use num_leaves (Not normally distributed)
cor.test(plot.plant$num_leaves, plot.plant$dand_diam, 
         method = 'spearman',
         alternative = 'two.sided')
```
...
6. Based on both tests, what do you conclude about the correlation (positive, negative, none) between number of leaves in a dandelion rosette and the diameter of a dandelion rosette?  
 > both tests give significant positive correlation (p < 0.05) between # of leaves in rosette and diameter of rosette
 
#### Exercise 2: social spiders  
...
1. Clearly state your null and alternative hypotheses for a regression analysis.  
  H0 : no effect of height of the web on the number of spiders in colony
        B1(slope) = 0
  HA: effect of height of the web on the number of spiders in colony
      B2(slope) != 0
  
  
2. Make a scatterplot of the data. What stands out to you about this scatterplot?  
```{r}
spiders <- read.csv("~/Desktop/Classes/EEMB 146 S20/Week 7/Lab /Lab 7 Data files-20200510/spiders.csv")

plot(spiders$height.cm, spiders$number.spiders,
     xlab = 'height', ylab = 'number of spiders')
text(spiders$height.cm, spiders$number.spiders, 
     labels = rownames(spiders), pos = 1)
...
# one way of fitting linear regression 
spiders.lm <- lm(number.spiders~height.cm, data = spiders)
summary(spiders.lm)
plot(spiders.lm)
...
# diagnostic plot
pairs.panels(spiders, cor = FALSE, density = TRUE, lm = TRUE)
# based on plots, assumptions of normality and homogenity of variance are NOT met
# point 5 makes its so there are sever violation of normality assuptions notted by qqplot
...
```{r}
library(tidyverse)
spiders.subset <- subset(spiders, colony !=5)

# check if you subsetted correctly!
#View(spiders.subset)
dim(spiders) 
dim(spiders.subset) # check that you successfully dropped it 
...
# after dropping 5, our assumptions of normality looks decent
# but there is a wedge shaped pattern in the residual plot, indicating our homogeniety of variance assumption might not be met
# lets try an a transformation
# only transfrom the y because the violation in the residul plot
# residuals are caculation as y observation - fitted y
# therefore the transformation is applied to fix violation in the response variable

```
  
5. If necessary, try transforming your response and/or predictor variables. Report what transformations you tried and show the resulting diagnostic plots. Make sure you continue to exlude colony 5!  
```{r}
# only transfrom the y because the violation in the residul plot
# residuals are caculation as y observation - fitted y
# therefore the transformation is applied to fix violation in the response variable
...
summary(fit3)
# after log transformation num.spiders, it looks like equal variacne assumtion are met
# it might not be as normal, but we'll say its fine for no

# now use a linear regress model to test H0
```
...
```{r}
# Y (response) is number of spiders
# X (explanatory) is height
# Y = b + mX
log_spiders <- 2.621925 + 0.007166*230

# how many spiders in a web that is 230cm off the ground?
exp(log_spiders) # have to retransform your y variable

# p < 0.5, x significant predictor of y
# r^2 is .7416, which means 76% of variation in log number of spiders can be explained by the height the colony was off the ground
```
...
8. Is web height a signficiant predictor of the number of spiders in a colony? What is the value of the p-value that is allowing you to make that conclusion?  
```{r}
# p < 0.5, x significant predictor of y
```

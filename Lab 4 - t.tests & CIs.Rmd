...
2. Lab 4 due Wednesday April 29th by 11:59 PM
  - Exercise 1: dominant vs. weak hand "dexterity.csv"
      - Dr. Briggs Wednesday OH example of 1) NSD: No smoking data 2) streams will be helpful to solve this question
  - Exercise 2: Natural selection in birds "bumpus.csv"
  - Exercise 3: Robustness of t-test "robustness.R.Rmd" on GS
  
3. Watch GS video 'Lab 4 video tutorial: Checking normality assumptions'

4. Common Questions
  - what does 'skewed data look like'
  - unable to read path file for HW question Error: 'can't connect', 'file, rt'
  - y ~ x 
#Agenda
Together:
...
Individually:
1. Read the text to guide you through this lab! It is really helpful!
2. Work through HW and Lab 4 exercises
...
```
  ...
  - if p > 0.05, fail to reject null, we can be reasonably confident that our data are normal
...
Before we can test these hypotheses we need to ensure that our assumptions are met. You can test the normality assumption using any of the procedures that you already know (e.g. qqplots or the Shapiro Wilk statistic). 
- To test the homogeneity (equal) of variance assumption we will use something called *Levene's Test*. The *null hypothesis* of the Levene's test is that the variance between the two groups are equal. The *alternative hypothesis* is that they are not equal.  
...
```{r}
# visualize age of passengers on titanic
...
```{r}
titanic <- read.csv("titanic.csv") # recommend calling in dataset for every HW question
# make a boxplot
...
# code thats on GS
Dexterity <- read.csv("~/Desktop/dexterity.csv") 
names(Dexterity)
#library(tidyverse)
#library(dplyr)
weak <- subset(Dexterity,hand == "weak")
dominant <- subset(Dexterity, hand == "dominant")
DEXT <- data.frame(weak, dominant)
DEXT2 <-DEXT[,-c(1,4)]
names(DEXT2) <- c('timew', 'weak', 'timed','dominant')

```

```{r piazza examples for Q1}


dexterity$time[dexterity$hand=="weak"] #create new data section for each hand
dexterity$time[dexterity$hand=="dominant"] #create new data section for each hand

# OR

dominant = subset(dexterity, hand=="dominant")
weak  = subset(dexterity, hand=="weak")
diff_dexterity = dominant$time - weak$time

boxplot(y ~ x, data = dexterity)
...
# how it's on gaucho space
boxplot(DEXT2$timew, DEXT2$timed)
```
...
??data.frame
?cbind()
dexterity <- read.csv("~/Desktop/Classes/EEMB 146 S20/Week 4/Lab 4/Lab 4 data/dexterity.csv")
head(dexterity)
# for some reason students have gotten a different data set that does not split up hands i'm calling it dexterity.2
dexterity.2 <- read.csv("~/Desktop/dexterity.csv")

weak <- subset(dexterity, dexterity$hand=="weak")
dominant <- subset(dexterity, dexterity$hand=="dominant")

dexterity.3 <- data.frame(weak, dominant)


t.test(dexterity.3$weak, dominant, paired = TRUE)


#now, run the t-test on those groups
t.test(lived$Age, died$Age, var.equal = FALSE) 
# two-sample t-test with unequal variances (Welch's two-sample t-test)
?subset()
...
H0 : mean of survival = mean of dead
p > 0.05
reject your H0 , the means are different 

p <0.05, fail to reject your null, so could be confident in saying your data could normal
